...
static int r_mark_regions(struct SN_env * z) {
    ...
    {   int c1 = z->c; /* or, line 51 */
        if (in_grouping_U(z, g_v, 97, 252, 0)) goto lab1; /* grouping v, line 48 */
    ...
    lab1:
        z->c = c1;
        if (out_grouping_U(z, g_v, 97, 252, 0)) return 0; /* non v, line 53 */
...
static int r_R1(struct SN_env * z) {
    if (!(z->I[0] <= z->c)) return 0; /* $p1 <= <integer expression>, line 58 */
    return 1;
}
...
static int r_instrum(struct SN_env * z) {
    ...
    switch(among_var) {
        ...
        case 1:
            ...
            break;
        case 2:
            {   int ret = r_double(z); /* call double, line 79 */
                if (ret == 0) return 0;
                if (ret < 0) return ret;
            }
            break;
...
static int r_factive(struct SN_env * z) {
    ...
    switch(among_var) {
        ...
        case 1:
            ...
            break;
        case 2:
            {   int ret = r_double(z); /* call double, line 135 */
                if (ret == 0) return 0;
                if (ret < 0) return ret;
            }
            break;
